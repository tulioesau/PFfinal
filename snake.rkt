...
; objetivo: verifica se a lista de pontos comida está na lista cobra
...
; restart: ->void
; objetivo: reseta todas as variaveis para o estado original
...
(define atualiza-cobra (lambda () 
  ...
  (for ([block cobra]) (
    if (eq? block (car cobra)) 
      ...
      (draw-block dc (list-ref block 0) (list-ref block 1) "red")))))
...
;fonte
(send dc set-font (make-object font%  20 'modern))
...
(define timer (new timer%
  [notify-callback (lambda()
    (send dc clear)
    (send dc set-brush "black" 'solid)    ;mundo    
